...
from grporange.classifier import VipMovementPredictor

data_file = [
    ("large", "./data-map-large-history5.csv"),
    ("medium", "./data-map-medium-history5.csv"),
    ("small", "./data-map-small-history5.csv")
]

for type, file_path in data_file:
    # 1. Chargement des données
    df = pd.read_csv(file_path)  # Remplacer par le vrai chemin du fichier CSV
    ...
    num_epochs = 100  # par exemple, on peut ajuster selon la convergence
    ...
    torch.save(model.state_dict(), f"./grporange/classifier/weights/vip_movement_{type}_predictor.pth")
