...
class Node:
    ...
    def __repr__(self, depth=0):
        ...
        for node in self._next_nodes:
            result += f"\n{'\t' * (depth+1)}- {node.__repr__(depth=depth+1)}"
        return result


class BetterMultiPlayerBot():
    # Player interface :
    ...
    def decide(self):
        ...
        print(f"{self._model._tic} bettermultiplayerbot player-{self._id}({self._model.score(self._id)}): {actions}")
        #input()
        return actions
    ...
    def calc_all_paths(self, player_i, robot_i, from_i, to_i, step_t=0):
        ...
        next_tiles = self._model._map.neighbours(from_i)
