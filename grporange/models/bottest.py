...
class BotTest():
    ...
    def decide(self):
        ...
        if closest_mission is not None:
            # Modification ici pour utiliser le tuple (id, mission)
            mission_id, mission = closest_mission
            (next_move, next_tile) = self.moveToward(r1Position, mission.start)
            return f"move {robot_id} {next_move}"
        else:
    ...
    def tsp(self, player_id, robot_id, duration=3):
        ...
        while current_time - start_time < duration:
            ...
            if n >= 10000:
                break
            result_path, current_score = self.swap(current_score, result_path)
